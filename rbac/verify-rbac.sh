...
# identical guard in resource-server-hardening/verify-token-relay.sh and
...
if [ "$FAIL" -eq 0 ]; then
  echo "RBAC matrix holds. Re-run verify-e2e.sh and verify-resource-server.sh for the full green baseline."
else
