...
cat <<HINT
  docker compose -f infra/docker-compose.yml exec -T keycloak /opt/keycloak/bin/kcadm.sh \\
...
if [ "$SUB" != "STILL MISSING" ]; then
  ...
  docker compose -f "$DC_FILE" exec -T postgres psql -U easyshop -d easyshop_user \
