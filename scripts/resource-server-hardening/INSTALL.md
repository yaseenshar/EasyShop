...
./scripts/keycloak-setup/verify-keycloak.sh     # MUST remain 14/14
```
...
# Align with the scope name created by scripts/resource-server-hardening/provision-audience.sh:
...
```bash
mvn -pl infrastructure/api-gateway -f backend/pom.xml clean install
docker compose -f infra/docker-compose.yml up -d --build api-gateway   # --build is required —
                                           # a plain restart reuses the old jar (§5.2 lesson)
```
...
./scripts/resource-server-hardening/verify-token-relay.sh
```

## Step 8 — Full regression

```bash
./scripts/integration-verification/verify-e2e.sh
./scripts/resource-server-hardening/verify-resource-server.sh
```
