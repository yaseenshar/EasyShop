...
if [ "$GW_UP" = "yes" ]; then
  ...
  echo "    docker compose -f infra/docker-compose.yml ps            # what is up?"
  echo "    docker compose -f infra/docker-compose.yml up -d api-gateway user-service"
  echo "    docker compose -f infra/docker-compose.yml logs --tail=50 user-service"
  exit 1
fi
