...
DBID=$(docker compose -f "$DC_FILE" exec -T postgres psql -U easyshop -d easyshop_user -tAc \
...
if [ "$SUB" = "$DBID" ]; then
  ...
  docker compose -f "$DC_FILE" exec -T postgres psql -U easyshop -d easyshop_user -c \
