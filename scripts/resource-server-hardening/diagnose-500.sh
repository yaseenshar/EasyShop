...
DC logs --tail=200 user-service 2>/dev/null \
...
if DC logs user-service 2>/dev/null | grep -qi "GlobalExceptionHandler\|CommonAutoConfiguration"; then
  echo "  mentioned in logs - likely loaded"
else
  echo "  NOT mentioned. Verify the install:"
  echo "    a) file at backend/common/common-lib/src/main/resources/META-INF/spring/"
  ...
  echo "    b) cd backend && mvn -pl common/common-lib install && mvn clean install"
  echo "    c) docker compose -f infra/docker-compose.yml up -d --build user-service"
  echo "       <-- REBUILD, not restart (a plain restart reuses the old jar with"
  echo "       the old common-lib)"
fi
...
DC exec -T postgres psql -U easyshop -d easyshop_user \
