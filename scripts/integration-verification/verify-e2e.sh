...
# Run AFTER: docker compose -f infra/docker-compose.yml up + all 8 services started.
...
# guard in scripts/resource-server-hardening/verify-token-relay.sh for the story.
case "${TEST_PASS:-}" in
  \<*\>)
