...
# Run from the project root: ./scripts/keycloak-setup/verify-keycloak.sh
# Secrets are auto-loaded from the repo-root .env - no inline vars needed.
#
...
if [ "$BASE_CODE" = "000" ]; then
  ...
  echo "    1. docker compose -f infra/docker-compose.yml ps keycloak"
  ...
  echo "    2. docker compose -f infra/docker-compose.yml logs --tail=100 keycloak"
