...
# docker-compose.yml lives in infra/ (project-structure refactor), not this
# script's own directory or the repo root - every "docker compose" call below
# needs -f pointed at it explicitly, or Compose can't find the project at all.
...
for V in KEYCLOAK_GATEWAY_CLIENT_SECRET KEYCLOAK_REVIEW_SERVICE_CLIENT_SECRET; do
  if DC exec -T keycloak printenv "$V" >/dev/null 2>&1; then
